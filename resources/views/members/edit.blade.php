...
@section('content')
    ...
    <form method="POST" action="{{ route('members.update', $member) }}" class="row g-3">
        ...
        <div class="col-md-6">
            ...
            <input type="text" name="last_name" id="last_name" value="{{ old('last_name', $member->last_name) }}" class="form-control @error('last_name') is-invalid @enderror">
        ...
        <div class="col-md-6">
            ...
            <select name="status" id="status" class="form-select @error('status') is-invalid @enderror">
                <option value="active" @selected(old('status', $member->status) === 'active')>aktywny</option>
                <option value="inactive" @selected(old('status', $member->status) === 'inactive')>nieaktywny</option>
            </select>
