...
@section('content')
    ...
    <form method="POST" action="{{ route('weapons.update', $weapon) }}" class="row g-3">
        ...
        <div class="col-md-6">
            ...
            <select name="weapon_type_id" id="weapon_type_id" class="form-select @error('weapon_type_id') is-invalid @enderror">
                ...
                @foreach ($weaponTypes as $weaponType)
                    <option value="{{ $weaponType->id }}" @selected((string) old('weapon_type_id', $weapon->weapon_type_id) === (string) $weaponType->id)>{{ $weaponType->name }}</option>
                @endforeach
            </select>
        ...
        <div class="col-md-6">
            ...
            <input type="text" name="name" id="name" value="{{ old('name', $weapon->name) }}" class="form-control @error('name') is-invalid @enderror">
            @error('name')<div class="invalid-feedback">{{ $message }}</div>@enderror
        </div>

        <div class="col-md-6">
            <label for="caliber" class="form-label">Kaliber</label>
            <input type="text" name="caliber" id="caliber" value="{{ old('caliber', $weapon->caliber) }}" class="form-control @error('caliber') is-invalid @enderror">
        ...
        <div class="col-md-6">
            ...
            <input type="text" name="serial_number" id="serial_number" value="{{ old('serial_number', $weapon->serial_number) }}" class="form-control @error('serial_number') is-invalid @enderror">
